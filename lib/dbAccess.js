...
module.exports = (function() {
    ...
    var couch=null,	StakeMinAge=2592000,
    getDb = function( ) { 
    //this keeps level open for the lifespan of the webserver, change this if you want to close on each request
      if (couch != null){
        return couch;
      }
      couch = level('./ppcdb', {
...
//;    
    ...
    var getTx = function (mpouts, cbWhenDone) {
        ...
        mpouts[txo].PrevTxOutIndex = parseInt(txo.slice(67),10);
            ...
            getDb().get('bh'+val.bh, function (err, value) {
              if (err) 
              {
      ...
      getDb().get(Settings.config.db.dbmetakey, function (err, val) {
        if (err) { // likely the key was not found
          ...
          callback(err, returnval);    
            ...
            getDb().get('bh'+val.MaxBStakemodifier, function (err, blk) {
              if (err) { // likely the key was not found
