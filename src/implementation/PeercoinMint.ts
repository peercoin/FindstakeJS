...
export class PeercoinMint {
  ...
  static BigToCompact(n: BN): number {
    ...
    let mantissa: number; // uint32   var	mantissa = compact & 0x007fffff,

    let exponent = n.toArray().length;
    if (exponent <= 3) {
      mantissa = n.and(new BN("4294967295", 10)).toNumber();
      mantissa <<= 8 * (3 - exponent);
    } else {
      // Use a copy to avoid modifying the caller's original number.
      let tn = new BN(n.toString(10), 10);
      mantissa = tn
        .shrn(8 * (exponent - 3))
  ...
  static CompactToDiff(bits: number): number {
    ...
    let diff = (1.0 * 0x0000ffff) / (bits & 0x00ffffff);
    for (let n = 0; nShift < 29; nShift++) {
      diff *= 256.0;
    }
    for (let n = 0; nShift > 29; nShift--) {
      diff /= 256.0;
